...
typedef struct flight3DConfig_s {
    ...
    uint16_t minsyncspeed3d;                // bidirectional 3d dshot RPM synchronization speed
    uint16_t minsyncspeedphasetwo3d;        // bidirectional 3d dshot RPM synchronization speed phase two
    uint8_t reversalrampuppwr3d;            // bidirectional 3d dshot RPM synchronization ramp-up power
    uint8_t reversalRPMdifferential3d;      // RPM differential check to determine if motors are reversed when checking RPMs
    uint8_t stabilizeTimeMs3d;              // Motor stabilization time after reversal sync is complete
    uint8_t throttleStepDelay3d;            // Number of steps * 10 to reach adjusted throttle stick value
    uint16_t pidResetTimeMs3d;              // PID reset time after reversal sync is complete
} flight3DConfig_t;
